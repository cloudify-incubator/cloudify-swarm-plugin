...
def start_service(**kwargs):
  ...
  if ctx.node.properties['compose_file'] != '':
    ...
    ctx.logger.debug("getting compose file:{}".format(ctx.node.properties['compose_file']))
...
def add_microservice(**kwargs):
  if ctx.target.node.type == 'cloudify.nodes.DeploymentProxy':
    ctx.source.instance.runtime_properties['ip']=ctx.target.instance.runtime_properties['swarm_info']['manager_ip']
    ctx.source.instance.runtime_properties['port']=ctx.target.instance.runtime_properties['swarm_info']['manager_port']
  else:
    ctx.logger.debug("in ad_microserivce")
